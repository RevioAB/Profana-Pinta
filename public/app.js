...
const products=[
{id:'santa',name:'SANTA',eyebrow:'EVERYDAY',type:'ALL PURPOSE SEASONING',line:'GARLIC • PAPRIKA • ONION • BLACK PEPPER',description:'The everyday spice blend for eggs, chicken, potatoes and roast vegetables. Savoury, warm and extremely easy to use.',best:'CHICKEN • EGGS • POTATOES',weight:'90 G',price:119,tone:'yellow',badge:'01',uses:['EGGS','CHICKEN','POTATOES','VEG'],tag:'ALL PURPOSE / ALL PANTRY'},
{id:'diabla',name:'DIABLA',eyebrow:'SMOKY HEAT',type:'SMOKY CHILLI SEASONING',line:'SMOKED CHILLI • GARLIC • LIME • CUMIN',description:'A smoky chilli spice blend with bright lime and cumin. Built for tacos, steak, beans and crispy potatoes.',best:'TACOS • BEEF • FRIES',weight:'90 G',price:119,tone:'red',badge:'02',uses:['TACOS','BEEF','FRIES','BEANS'],tag:'SMOKY / BRIGHT / BOLD'},
{id:'verde',name:'VERDE',eyebrow:'BRIGHT HERB',type:'HERB AND LEMON SEASONING',line:'HERBS • LEMON • GARLIC • BLACK PEPPER',description:'A fresh herb seasoning with lemon and garlic for pasta, fish, halloumi, salads and anything that needs waking up.',best:'PASTA • FISH • HALLOUMI',weight:'90 G',price:119,tone:'green',badge:'03',uses:['PASTA','FISH','SALAD','HALLOUMI'],tag:'HERBY / ZIPPY / BRIGHT'}
];
...
const tickerCopy=['SEASON YOUR EGGS','SEASON YOUR POTATOES','SEASON YOUR CHICKEN','SEASON YOUR EVERYTHING'];$$('.ticker span').forEach((el,i)=>el.textContent=tickerCopy[i]);
$('.intro .kicker').textContent='THREE SPICE BLENDS. ONE BETTER PANTRY.';
$('.intro>p').innerHTML='Santa, Diabla and Verde are everyday seasoning blends made to answer one question fast: <i>what does this need?</i>';
const trio=$('.trioCopy');trio.querySelector('.kicker').textContent='THE STARTER SPICE TRIO';trio.querySelector('h2').innerHTML='Three seasoning blends.<br>One easy start.';trio.querySelector('p:not(.kicker)').textContent='Three 90 g seasoning blends that cover the food you actually cook. Start here and your pantry is already more useful.';trio.querySelector('ul').innerHTML='<li><b>SANTA</b><span>all purpose seasoning for eggs, chicken and potatoes</span></li><li><b>DIABLA</b><span>smoky chilli seasoning for tacos, beef and fries</span></li><li><b>VERDE</b><span>herb and lemon seasoning for pasta, fish and halloumi</span></li>';
$('.shop .kicker').textContent='SHOP THE SEASONING BLENDS';
$('.shop .sectionHead h2').textContent='Pick your flavour.';
$('.shop .sectionHead>p').textContent='Three clear flavour profiles. Three 90 g bottles. Made to stay beside the stove and get used every day.';

$('#productGrid').innerHTML=products.map(p=>`<article class="product ${p.tone}"><div class="productVisual">${bottle(p)}</div><div class="productInfo"><div class="productMeta"><span>${p.eyebrow} • ${p.weight}</span><span>${p.price} SEK</span></div><h3>${p.name}</h3><div class="productType">${p.type}</div><div class="flavour">${p.line}</div><p>${p.description}</p><div class="bestOn"><b>BEST ON</b>${p.best}</div><button class="add" data-add="${p.id}">ADD SEASONING TO BAG</button></div></article>`).join('');
